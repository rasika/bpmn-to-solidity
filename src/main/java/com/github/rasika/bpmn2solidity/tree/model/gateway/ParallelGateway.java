...
public class ParallelGateway extends SolidityNode {
    ...
    public void toSolidity(SolidityCodeTemplate template) throws SolidityParserException {
        System.out.println("// ------ParallelGW:" + id);
        if (isConverging()) {
            ...
            List<SequenceFlow> flows = process.sequenceFlows.stream().filter(s -> id.equals(s.sourceRef)).collect(
                    Collectors.toList());
            ...
            if (topLevelTask != null) {
                ...
                List<SequenceFlow> flows = process.sequenceFlows.stream().filter(s -> id.equals(s.sourceRef)).collect(
                        Collectors.toList());
                processIfConditions(template, contract.getConstructor(), flows);
            }
        }
    }

    private void processIfConditions(SolidityCodeTemplate template, AbstractFunction function, List<SequenceFlow> flows)
        ...
        Comparator<SequenceFlow> byFirst = comparing(
                f -> f.getChildren().stream().anyMatch(c -> "conditionExpression".equals(c.type)), reverseOrder());
    ...
    private void validateGateway(List<SequenceFlow> flows, String defaultSeq) throws SolidityParserException {
        ...
        temp.removeIf(f -> (f.getChildren().stream().anyMatch(c -> "conditionExpression".equals(c.type)) ||
                (defaultSeq != null && defaultSeq.equals(f.id))));
        ...
        if (temp.size() > 1 && !isDiverging) {
            ...
            throw new SolidityParserException(
                    "Exclusive Gateway:" + id + " has more than one else cases!" + suggestion);
        }
    }
    ...
    private boolean isConverging() {
        Process process = getParentProcess(idToNode);
        List<SequenceFlow> outFlows = process.sequenceFlows.stream().filter(s -> id.equals(s.sourceRef)).collect(
                Collectors.toList());
        List<SequenceFlow> inFlows = process.sequenceFlows.stream().filter(s -> id.equals(s.targetRef)).collect(
                Collectors.toList());
